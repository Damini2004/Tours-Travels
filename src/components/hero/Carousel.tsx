...
// initialImages definition should match the one in user's provided code
const initialImages = [
...
const Carousel = () => {
  ...
  const timeAutoNext = 7000; // As per previous working version
  ...
  return (
    <>
      <div className='mt-[-160px] w-full h-[105vh] bg-gradient-to-br from-[#031f2d] via-[#0c4d52] to-[#155e63] overflow-hidden relative z-0'>
        <div className={cn(`w-full h-full relative overflow-hidden`, transitionState === 'next' ? 'next' : transitionState === 'prev' ? 'prev' : '')}>
          <div className="w-full h-full relative">
            {items.map((item, index) => {
              
              let itemBaseClasses = "absolute bg-center bg-cover transition-all duration-1000 ease-in-out";
              let itemDynamicStyle: React.CSSProperties = { backgroundImage: `url(${item.image})` };
              let contentVisibilityClass = "hidden";
              let zIndex = "z-[100]";

              if (index === 0 || index === 1) { 
                itemBaseClasses += " top-0 left-0 w-full h-full rounded-none transform-none"; // transform-none to override potential inherited transforms
                zIndex = index === 0 ? "z-10" : "z-[9]";
                if (index === 1) {
                  contentVisibilityClass = "block"; 
                }
              } else {
                itemBaseClasses += " w-[180px] h-[250px] top-[80%] rounded-[20px] shadow-[0_25px_50px_rgba(0,0,0,0.3)]";
                itemDynamicStyle.transform = 'translateY(-70%)';
                if (index === 2) itemDynamicStyle.left = '67%';
                else if (index === 3) itemDynamicStyle.left = 'calc(67% + 200px)';
                else if (index === 4) itemDynamicStyle.left = 'calc(67% + 400px)';
                else if (index === 5) itemDynamicStyle.left = 'calc(67% + 600px)';
                else { 
                  itemDynamicStyle.left = 'calc(67% + 800px)';
                  itemBaseClasses += " opacity-0";
                }
              }
              ...
              return (
                <div 
                  ...
                  className={cn(itemBaseClasses, zIndex)} 
                  style={itemDynamicStyle}
                  data-ai-hint={item.hint}
                >
                  <div className={cn(
                      "absolute top-1/2 left-[30px] md:left-[100px] -translate-y-1/2 w-[calc(100%-60px)] p-5 bg-black/50 rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.8)] z-20", // Adjusted width for sm screens
                      "md:w-[500px] md:max-w-[600px]", // md and larger screens
                      "max-[400px]:mt-[70px] max-[400px]:w-[70vw]", // Specific small screen from CSS
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      <button className={cn(
                          ...
                          "max-[690px]:py-[10px] max-[690px]:px-[15px] max-[690px]:text-sm"
                      )}>
                        See More
                      </button>
                    </div>
          ...
          <div className="absolute bottom-[30px] left-1/2 -translate-x-1/2 z-[100] flex gap-[20px] p-[12px_20px] rounded-[24px] animate-float">
            <button 
              className="prev w-[56px] h-[56px] rounded-full text-white bg-transparent border-2 border-white/85 outline-none text-2xl font-orbitron font-extrabold cursor-pointer transition-all duration-300 ease hover:[transform:perspective(600px)_rotateX(0deg)] active:[transform:perspective(600px)_rotateX(0deg)]" // Added active state
              onClick={() => showSlider('prev')}
            >
              {'<'}
            </button>
            <button 
              className="next w-[56px] h-[56px] rounded-full text-white bg-transparent border-2 border-white/85 outline-none text-2xl font-orbitron font-extrabold cursor-pointer transition-all duration-300 ease hover:[transform:perspective(600px)_rotateX(0deg)] active:[transform:perspective(600px)_rotateX(0deg)]" // Added active state
          ...
          <div 
            className="absolute bottom-0 left-0 h-1.5 bg-gradient-to-r from-sky-400 via-rose-400 to-lime-400 z-[100]" 
            ref={animationRef}
            style={{ width: '0%' }} 
          ></div>
        </div>
