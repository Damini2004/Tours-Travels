...
function ReviewBookingContent() {
    ...
    return (
        <div className="min-h-screen bg-background">
            <div className="bg-[#031f2d]/90 backdrop-blur-sm text-white py-4 sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-4">
                    <h1 className="text-2xl font-semibold mb-3 text-white">Complete your booking</h1>
            ...
            <main className="max-w-6xl mx-auto px-4 py-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-6">
                        ...
                        <Card className="bg-card shadow-lg border-border overflow-hidden">
                            <CardContent className="p-4 md:p-5">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <h2 className="text-xl font-bold text-card-foreground">{getAirportDisplayName(segments[0]?.departureIata || origin || "", 'city')} → {getAirportDisplayName(segments[segments.length - 1]?.arrivalIata || destination || "", 'city')}</h2>
                                ...
                                <div className="border border-border rounded-md p-3">
                                    ...
                                    {segments.map((segment, idx) => (
                                        <div key={idx} className="relative pl-5 mb-3 last:mb-0 group text-sm">
                                            <div className="absolute left-0 top-[5px] w-2.5 h-2.5 bg-primary rounded-full border-2 border-card z-10 shadow"></div>
                                            ...
                                            <div className="flex items-baseline">
                                                <span className="font-semibold text-card-foreground w-12 shrink-0">{segment.departureAt ? format(parseISO(segment.departureAt), "HH:mm") : ''}</span>
                                            ...
                                            <div className="flex items-baseline">
                                                <span className="font-semibold text-card-foreground w-12 shrink-0">{segment.arrivalAt ? format(parseISO(segment.arrivalAt), "HH:mm") : ''}</span>
                                    ...
                                    <div className="mt-3 text-xs text-primary bg-primary/10 p-2 rounded-md flex justify-between items-center border border-primary/20">
                                        <div className="flex items-start">
                                            <Image src="https://placehold.co/20x20/64B5F6/FFFFFF.png?text=!" alt="info" width={16} height={16} className="inline mr-1.5 shrink-0 mt-px rounded-full" data-ai-hint="info icon"/>
                        ...
                        <Card className="bg-card shadow-lg border-border">
                            <CardHeader className="pb-2 pt-4 px-4 md:px-5">
                                <div className="flex justify-between items-center">
                                    <CardTitle className="text-base font-bold text-card-foreground">Cancellation & Date Change Policy</CardTitle>
                            ...
                            <CardContent className="px-4 md:px-5 pb-4 text-sm">
                                ...
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <span className="text-sm font-semibold text-card-foreground">₹ {cancellationFeeNow !== null ? cancellationFeeNow.toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits: 0}) : '...'}</span>
                                </div>
                                <div className="my-4">
                                    <div className="relative h-1.5 bg-muted rounded-full">
                                        ...
                                        <div className="absolute -top-4 left-[calc(60%-1.5rem)] text-center w-12 text-xs font-semibold text-card-foreground">
                                            ₹{cancellationFeeNow !== null ? cancellationFeeNow.toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits: 0}) : '...'}
                                        </div>
                                         <div className="absolute -top-4 right-[0.5rem] text-center w-12 text-xs font-semibold text-card-foreground">
                    ...
                    <div className="space-y-6">
                        <Card className="bg-card shadow-lg border-border sticky top-24"> 
                            <CardHeader className="pb-3 pt-4">
                                <CardTitle className="text-base font-bold text-card-foreground">Fare Summary</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-2 text-sm pb-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-medium text-card-foreground">₹ {clientBaseFare !== null ? clientBaseFare.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '...'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-medium text-card-foreground">₹ {clientTaxesAndSurcharges !== null ? clientTaxesAndSurcharges.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '...'}</span>
                                </div>
                                <Separator className="my-3 bg-border" />
                                <div className="flex justify-between text-base">
                                    <span className="font-semibold text-card-foreground">Total Amount</span>
                                    <span className="font-bold text-card-foreground">₹ {clientTotalFare !== null ? clientTotalFare.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '...'}</span>
                                </div>
                            </CardContent>
                        </Card>

                        <Card className="bg-card shadow-lg border-border">
                            <div className="bg-gradient-to-r from-orange-400 via-amber-400 to-yellow-500 p-3 rounded-t-lg">
                                <CardTitle className="text-base flex items-center gap-2 font-bold text-black">
                                    <Image src="https://placehold.co/24x24/000000/FFFFFF.png?text=🎁" alt="coupons" width={20} height={20} className="rounded-sm" data-ai-hint="giftbox present"/> Coupons and Offers
                                </CardTitle>
                            </div>
    ...
    return (
        <Suspense fallback={
            <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
                ...
                <h2 className="text-xl font-semibold text-foreground mb-2">Loading Your Booking Details...</h2>
