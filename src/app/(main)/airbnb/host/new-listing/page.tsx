...
import { HomeIcon as PropertyIcon, PlusCircleIcon, UploadIcon, Loader2, BedIcon, UsersIcon, BathIcon, TagIcon, MapPinIcon } from "lucide-react";
import { useState, useEffect } from "react";
import { useToast } from "@/hooks/use-toast";
import { addProperty } from '@/lib/airbnb-data'; // New data handler
...
export default function NewListingPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser) {
        toast({ variant: "destructive", title: "Unauthorized", description: "You must be logged in to list a property." });
        return;
    }
    ...
    const newPropertyData: Omit<Property, 'id' | 'rating' | 'hostAvatarUrl' | 'isSuperhost'> = {
        ...
        hostEmail: currentUser.email, // Assuming you add hostEmail to Property type
    ...
    try {
      ...
      router.push("/airbnb"); // Redirect to Airbnb main page
    } catch (error) {
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input id="title" placeholder="e.g., Cozy Downtown Studio with Great Views" required value={title} onChange={(e) => setTitle(e.target.value)} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <Input id="location" placeholder="e.g., Paris, France" required value={location} onChange={(e) => setLocation(e.target.value)} className="pl-10" />
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <Input id="pricePerNight" type="number" placeholder="e.g., 100" required value={pricePerNight} onChange={(e) => setPricePerNight(e.target.value)} className="pl-10" min="1"/>
                    </div>
                </div>
                <div className="space-y-2">
                    <Label htmlFor="guests">Max Guests *</Label>
                     <div className="relative">
                        ...
                        <Input id="guests" type="number" placeholder="e.g., 2" required value={guests} onChange={(e) => setGuests(e.target.value)} className="pl-10" min="1"/>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <Input id="bedrooms" type="number" placeholder="e.g., 1" required value={bedrooms} onChange={(e) => setBedrooms(e.target.value)} className="pl-10" min="0"/>
                    </div>
                </div>
                <div className="space-y-2">
                    <Label htmlFor="beds">Beds *</Label>
                     <div className="relative">
                        <BedIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                        <Input id="beds" type="number" placeholder="e.g., 1" required value={beds} onChange={(e) => setBeds(e.target.value)} className="pl-10" min="1"/>
                    </div>
                </div>
                <div className="space-y-2">
                    <Label htmlFor="baths">Bathrooms *</Label>
                     <div className="relative">
                        ...
                        <Input id="baths" type="number" placeholder="e.g., 1" required value={baths} onChange={(e) => setBaths(e.target.value)} className="pl-10" min="0.5" step="0.5"/>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="description" placeholder="Tell travelers about your space: what makes it unique, the neighborhood, etc." value={description} onChange={(e) => setDescription(e.target.value)} rows={5}/>
            </div>

            <div className="space-y-2">
              <Label htmlFor="amenities">Amenities (comma-separated)</Label>
              <Input id="amenities" placeholder="e.g., Wifi, Kitchen, Free parking, Pool" value={amenities} onChange={(e) => setAmenitiesState(e.target.value)} />
            </div>
                ...
                <div className="flex items-center justify-center w-full">
                    ...
                    >
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="text-xs text-muted-foreground">SVG, PNG, JPG (Not functional)</p>
                        </div>
